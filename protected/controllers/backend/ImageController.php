...
class ImageController extends Controller {
            ...
            if (isset($images) && count($images) > 0) {
                foreach ($images as $image => $pic) {
                    if ($pic->saveAs(Yii::getPathOfAlias('webroot') . '/anhUpload/' . $pic->name)) {
                        ...
                        $model->imageId = null;
                        $model->imageName = $pic->name;
                        $model->imageUrl = '/../anhUpload/' . $pic->name;
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Image'])) {
            ...
            $model->imageUrl = '/../anhUpload/' . $model->imageName;
            $model->createDate = date("Y-m-d H:i:s");
            if ($model->save()) {
                $model->imageName->saveAs(Yii::app()->basePath . '/../anhUpload/' . $model->imageName);
                $this->redirect(array('view', 'id' => $model->imageId));
            }
        }
    ...
    public function actionDelete($id) {
        ...
        $model = $this->loadModel($id);
        @unlink(Yii::app()->basePath . $model->imageUrl);
